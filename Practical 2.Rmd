...
There are six variables in the dataset and they are briefly described below.

*note_id:* A unique identifier for each medical note.
*description:* A description of the patient concern associated with the note.
*medical_specialty:* The medical field or specialty to which the note belongs.
*sample_name:* The name of the type of disease or procedure.
*transcription:* The details of the disease or procedure.
*keywords:* Keywords associated with the disease or procedure.
...
analysis.data <- filtered.data %>%
  ...
  left_join(select(filtered.data, -transcription), by = "note_id", copy = TRUE)
```
...
```{r}
# Count the unique bigrams for each medical specialty
unique_bigrams <- tokenized.data %>%
  group_by(medical_specialty) %>%
  summarise(unique_bigrams = n_distinct(ngram))

# Display the unique bigrams count per medical specialty
unique_bigrams
```
...
```{r}
# Filter the data for the specified specialties
...
tokenized.data_sentences <- filtered.data %>%
  unnest_tokens(sentence, transcription, token = "sentences")

# Count unique sentences per specialty
unique_sentences_per_specialty <- tokenized.data_sentences %>%
  group_by(medical_specialty) %>%
  summarise(num_unique_sentences = n_distinct(sentence))

# Print the result
unique_sentences_per_specialty
```
...
A general purpose lemmatizer may not work well for medical data due to the specialized and complex nature of medical terminology, which includes unique vocabulary, abbreviations, and context-sensitive terms. Medical texts often use complex morphology and context-specific meanings that general lemmatizers are not equipped to handle. Additionally, medical data benefits from integration with specialized ontologies like SNOMED CT, which general lemmatizers typically do not support. Therefore, using domain-specific lemmatizers or hybrid approaches that incorporate medical knowledge is essential for accurate processing of medical data.
...
The scatter plot comparing Surgery and Orthopedic shows many lemmas have similar frequencies, with some differences. In contrast, the Radiology vs. Orthopedic plot has more spread, indicating greater differences in lemma frequencies. This makes sense because Surgery and Orthopedics share more common terms, while Radiology uses more specialized language due to its focus on imaging.
...
lemma.freq <- lemmatized.data %>% 
  ...
  dplyr::mutate(proportion = n / sum(n)) %>%
  tidyr::pivot_wider(names_from = medical_specialty, values_from = proportion) %>%
  tidyr::pivot_longer(cols = c(`Orthopedic`, `Radiology`), 
                      names_to = "medical_specialty", 
...
```
As expected, the Surgery vs Orthopedic scatter plot shows lemmas having similar frequencies as most of the points cluster around the line. This represents common lemmas shared by both specialties. Conversely, points on the Surgery vs Radiology deviate quite a lot from the line as these specialties might have minimal overlapping terms.
...
Common lemmas are frequent across various surgical procedures, resulting in low tf-idf values due to their ubiquity. In Orthopedics, tf-idf values are uniformly low, indicating common lemmas throughout, so none stand out. Radiology shows more unique lemmas with higher tf-idf values, highlighting its specialized terms, such as "left ventricular." In Surgery, some lemmas like "general endotracheal" and "fascia was" have high tf-idf values, standing out due to their specificity.
...
analysis.data %>%
  ...
  dplyr::filter(!stringr::str_detect(medical_specialty, 'Surgery')) %>%
  dplyr::slice(1)
```
...
# Create a mapping of document IDs to medical specialties
note_id_specialty_mapping <- lemmatized.data %>%
  dplyr::mutate(document=as.character(note_id)) %>% 
  dplyr::select(document, medical_specialty) %>% 
  dplyr::distinct()
...
# Visualize the distribution of topics across medical specialties
specialty_gamma6 %>%
  dplyr::mutate(medical_specialty = reorder(medical_specialty, gamma * topic)) %>%
  ggplot2::ggplot(ggplot2::aes(factor(topic), gamma)) +
  ggplot2::geom_boxplot() +
